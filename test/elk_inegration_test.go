...
func TestElkIntegration(t *testing.T) {
	...
	helmChartPath, err := filepath.Abs(filepath.Join("..", "charts", "icon-prep"))
	require.NoError(t, err)

	kubectlOptions := k8s.NewKubectlOptions("", "", "")
	uniqueID := random.UniqueId()
	testNamespace := fmt.Sprintf("icon-prep-stack-%s", strings.ToLower(uniqueID))
	...
	releaseName := fmt.Sprintf("icon-prep-stack-%s", strings.ToLower(uniqueID))
